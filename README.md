...
若要先用 OpenAI 語音轉文字 API 從官方音檔產生初稿，可設定 `OPENAI_API_KEY` 後執行：

```bash
OPENAI_API_KEY=... scripts/transcribe-openai.sh 51
```

腳本會下載指定 Track 到 `media-cache/`，並把轉錄文字輸出到 `private/transcripts/`。這兩個目錄已被 `.gitignore` 排除，避免把音檔或完整轉錄稿放進公開 repo。
